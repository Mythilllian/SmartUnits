...
if TYPE_CHECKING:
        from smartunits import LuminousFluxUnit 

@dataclass(frozen=True, slots=True)
class LuminousFlux(Measure["LuminousFluxUnit"], ABC):
    _magnitude: float
    _base_unit_magnitude: float
    _unit: "LuminousFluxUnit"

    @staticmethod
    def of(magnitude: float, unit: "LuminousFluxUnit") -> "LuminousFlux":
        return LuminousFlux(magnitude, unit.to_base_units(magnitude), unit)

    @staticmethod
    def of_base_units(base_unit_magnitude: float, unit: "LuminousFluxUnit") -> "LuminousFlux":
        return LuminousFlux(unit.from_base_units(base_unit_magnitude), base_unit_magnitude, unit)
    ...
    @override
    def unit(self) -> "LuminousFluxUnit":
        return self._unit
    ...
    @override
    def __neg__(self) -> "LuminousFlux":
        return LuminousFlux(-self._magnitude, -self._base_unit_magnitude, self._unit)
    
    @override
    def __add__(self, other: Measure["LuminousFluxUnit"]) -> "LuminousFlux":
        if self._unit is other._unit:
            return LuminousFlux(self._magnitude + other._magnitude, self._base_unit_magnitude + other._magnitude, self._unit)
        
        return self._unit.of_base_units(self._base_unit_magnitude + other._base_unit_magnitude)
    
    @override
    def __sub__(self, other: Measure["LuminousFluxUnit"]) -> "LuminousFlux":
        if self._unit is other._unit:
            return LuminousFlux(self._magnitude - other._magnitude, self._base_unit_magnitude, self._unit)
    ...
    def __mul__(self, other: Any) -> Measure[Any]:
        ...
        if isinstance(other, (int, float)):
                return LuminousFlux(self._magnitude * other, self._base_unit_magnitude * other, self._unit)

        # handles LuminousFlux
        if isinstance(other, LuminousFlux):
        ...
        if isinstance(other, Dimensionless):
                ...
                return LuminousFlux(self._magnitude * factor, self._base_unit_magnitude * factor, self._unit)

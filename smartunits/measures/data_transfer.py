...
if TYPE_CHECKING:
        from smartunits import DataTransferUnit 

@dataclass(frozen=True, slots=True)
class DataTransfer(Measure["DataTransferUnit"], ABC):
    _magnitude: float
    _base_unit_magnitude: float
    _unit: "DataTransferUnit"

    @staticmethod
    def of(magnitude: float, unit: "DataTransferUnit") -> "DataTransfer":
        return DataTransfer(magnitude, unit.to_base_units(magnitude), unit)

    @staticmethod
    def of_base_units(base_unit_magnitude: float, unit: "DataTransferUnit") -> "DataTransfer":
        return DataTransfer(unit.from_base_units(base_unit_magnitude), base_unit_magnitude, unit)
    ...
    @override
    def unit(self) -> "DataTransferUnit":
        return self._unit
    ...
    @override
    def __neg__(self) -> "DataTransfer":
        return DataTransfer(-self._magnitude, -self._base_unit_magnitude, self._unit)
    
    @override
    def __add__(self, other: Measure["DataTransferUnit"]) -> "DataTransfer":
        if self._unit is other._unit:
            return DataTransfer(self._magnitude + other._magnitude, self._base_unit_magnitude + other._magnitude, self._unit)
        
        return self._unit.of_base_units(self._base_unit_magnitude + other._base_unit_magnitude)
    
    @override
    def __sub__(self, other: Measure["DataTransferUnit"]) -> "DataTransfer":
        if self._unit is other._unit:
            return DataTransfer(self._magnitude - other._magnitude, self._base_unit_magnitude, self._unit)
    ...
    def __mul__(self, other: Any) -> Measure[Any]:
        ...
        if isinstance(other, (int, float)):
                return DataTransfer(self._magnitude * other, self._base_unit_magnitude * other, self._unit)

        # handles DataTransfer
        if isinstance(other, DataTransfer):
        ...
        if isinstance(other, Dimensionless):
                ...
                return DataTransfer(self._magnitude * factor, self._base_unit_magnitude * factor, self._unit)

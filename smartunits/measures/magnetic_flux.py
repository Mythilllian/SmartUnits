...
if TYPE_CHECKING:
        from smartunits import MagneticFluxUnit 

@dataclass(frozen=True, slots=True)
class MagneticFlux(Measure["MagneticFluxUnit"], ABC):
    _magnitude: float
    _base_unit_magnitude: float
    _unit: "MagneticFluxUnit"

    @staticmethod
    def of(magnitude: float, unit: "MagneticFluxUnit") -> "MagneticFlux":
        return MagneticFlux(magnitude, unit.to_base_units(magnitude), unit)

    @staticmethod
    def of_base_units(base_unit_magnitude: float, unit: "MagneticFluxUnit") -> "MagneticFlux":
        return MagneticFlux(unit.from_base_units(base_unit_magnitude), base_unit_magnitude, unit)
    ...
    @override
    def unit(self) -> "MagneticFluxUnit":
        return self._unit
    ...
    def __neg__(self) -> "MagneticFlux":
        return MagneticFlux(-self._magnitude, -self._base_unit_magnitude, self._unit)
    
    @override
    def __add__(self, other: Measure["MagneticFluxUnit"]) -> "MagneticFlux":
        if self._unit is other._unit:
            return MagneticFlux(self._magnitude + other._magnitude, self._base_unit_magnitude + other._magnitude, self._unit)
        
        return self._unit.of_base_units(self._base_unit_magnitude + other._base_unit_magnitude)
    
    @override
    def __sub__(self, other: Measure["MagneticFluxUnit"]) -> "MagneticFlux":
        if self._unit is other._unit:
            return MagneticFlux(self._magnitude - other._magnitude, self._base_unit_magnitude, self._unit)
    ...
    def __mul__(self, other: Any) -> Measure[Any]:
        ...
        if isinstance(other, (int, float)):
                return MagneticFlux(self._magnitude * other, self._base_unit_magnitude * other, self._unit)

        # handles MagneticFlux
        ...
        if isinstance(other, Dimensionless):
                ...
                return MagneticFlux(self._magnitude * factor, self._base_unit_magnitude * factor, self._unit)

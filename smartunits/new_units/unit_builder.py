...
class UnitBuilder(Generic[U]):
    ...
    class MappingBuilder:
        ...
        def __init__(self, unit_builder: "UnitBuilder[U]", min_input: float, max_input: float):
            self._min_input = min_input
            self._max_input = max_input
            self._unit_builder = unit_builder

        def to_output_range(self, min_output: float, max_output: float) -> "UnitBuilder.MappingBuilder":
            self._unit_builder._from_base = lambda x: self._unit_builder.map_value(x, self._min_input, self._max_input, min_output, max_output)
            self._unit_builder._to_base = lambda x: self._unit_builder.map_value(x, min_output, max_output, self._min_input, self._max_input)
            self._unit_builder._offset = 0
            return self
        
    def mapping_input_range(self, min_base: float, max_base: float) -> "UnitBuilder.MappingBuilder":
        return self.MappingBuilder(self, min_base, max_base)
    
    def set_conversion_functions(self, from_base: Callable[[float], float], to_base: Callable[[float], float]) -> "UnitBuilder[U]":
    ...
    def split_into(self, fraction: float):
        ...
        return self.set_conversion_functions(lambda b:b*fraction, lambda x:x/fraction)

    def aggregate(self, aggregation: float) -> "UnitBuilder[U]":
        if aggregation == 0:
            raise ValueError("Aggregation amount must be nonzero")

        return self.set_conversion_functions(lambda b:b/aggregation, lambda x:x*aggregation)

    class UnitConstructorFunction(ABC, Generic[U]):
        @abstractmethod
        def create(base_unit: U, to_base_units: Callable[[float],float], from_base_units: Callable[[float],float], name: str, symbol: str):
            pass

    def make(self, constructor: UnitConstructorFunction[U]) -> Type[U]:
    ...
    def make(self) -> Type[U]:
        return self.make(self._base.__class__)
    
    @staticmethod
    def get_constructor(base_unit: U) -> Type[U]:
